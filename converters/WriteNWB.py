...
def create_electrode_table_region_old(nwbfile, header, intan_device):
...
def create_electrode_table_region(nwbfile, header, intan_device, electrode_mappings=None, electrode_headers=None):
    #DUANE MODIFIED 1-DEC-2022

    # CREATE DEVICE TO STORE ELECTRODES DATA
    device_mapping = nwbfile.create_device(
        name=electrode_headers['electrode_device_name'], description=electrode_headers['electrode_recordings_description'], manufacturer='Intan'
    )
    ...
    # Create an electrode group for each port
    if header['num_amplifier_channels'] > 0:
        #print(f"number of channels: {header['num_amplifier_channels']}")
        nwbfile.add_electrode_column(name='imp_phase',
                                     description='phase (in degrees) of complex impedance of this channel')
        nwbfile.add_electrode_column(name='native_channel_name',
                                     description='native, uneditable name (for example, A-000) of this channel')
        #CUSTOM ADD (1-DEC-2022): ELECTRODE MAPPINGS [DEVICE CHANNEL:INTAN CHANNEL]
        nwbfile.add_electrode_column(name='electrode_mapping',
                                     description='electrode_mapping: device_channel, Intan_channel')
        created_electrode_groups = {}


    # create a single electrode group (to store mappings)
    electrode_group = nwbfile.create_electrode_group(
        name='electrode_mapping',
        description='electrode_mapping: device_channel, Intan_channel',
        device=device_mapping,
        location=electrode_headers['electrode_recordings_location']
    )
    created_electrode_groups['electrode_mapping'] = electrode_group
    ...
    # Create an electrode for each channel, and associate it with its port's group
    for channel in range(header['num_amplifier_channels']):
        this_channel_struct = header['amplifier_channels'][channel]
        #custom_channel_name = this_channel_struct['custom_channel_name']
        group_name = 'Intan ' + this_channel_struct['port_name'] + ' electrode group'
        nwbfile.add_electrode(id=channel,
                              ...
                              native_channel_name=this_channel_struct['native_channel_name'],
                              #custom_channel_name=this_channel_struct['custom_channel_name'],
                              location=str(electrode_headers['electrode_recordings_location']),
                              filtering=str(electrode_headers['electrode_filtering']),
                              electrode_mapping=str(electrode_mappings[channel]),
                              group=created_electrode_groups[group_name])


    # If there are any amplifier channels, create this region
    #1-DEC-2022 mod to include electrode mapping table
    if header['num_amplifier_channels'] > 0:
        electrode_table_region = nwbfile.create_electrode_table_region(list(range(0, header['num_amplifier_channels'])),
                                                                       'Intan electrode table region')
    else:
