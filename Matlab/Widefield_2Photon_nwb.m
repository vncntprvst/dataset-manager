...
clear; clc; close all;
output_path = "C:\tmp\output\";
input_path = "Y:\DataAnalysis\VesCorrPhase\Rui_2P\";
% Check if output_path exists, create it if not
if ~exist(output_path, 'dir')
    mkdir(output_path);
end

% ITERATE OVER SUBJECTS IN INPUT FORM %
subtable = readtable('Z:\Jacob\nwb_process\input_widefield_2P.xlsx');

% log2C = cell(length(subtable.age),1);
counter = 1;
warning('off','all')
for subj = 1:length(subtable.age)
% for subj = 23:23
    
    %Initialize subject information
    age = string(subtable.age(subj));
    ...
    subj_species = subtable.species(subj);
    subj_sex = subtable.sex(subj);
    subj_id = subtable.subject_id(subj);
    subj_description = subtable.subject_description(subj);
    subj_strain = subtable.subject_strain(subj);
    subj_DOB = subtable.date_of_birth_YYYY_MM_DD_(subj);
    subj_genotype = subtable.genotype(subj);
    % Determine which experiment data to include with this subject.
    subj_figs = subtable.session_description(subj);
    subj_session_id = subtable.session_id(subj);
    subj_surgery = subtable.surgery(subj);
    subj_stim = subtable.stimulus_notes(subj);
    ...
    nwb = NwbFile(...
        ...
        'general_experimenter', 'Duckworth, Jacob A.', ...
        'general_institution', 'University of California San Diego', ...
...
%     subj_figs = subj_figs{1}(1:20)
    %% Add device object
    subj_device = subtable.device_name(subj);
    subj_device_description = subtable.device_description(subj);
    % devices = types.core.DeviceCollection('description', 'Two-photon microscope with adaptive optics and piezo');
    ...
    if contains(subj_figs,' 1d')
        ...
        table = readtable("\\dk-server.dk.ucsd.edu\jaduckwo\DataAnalysis\VesCorrPhase\IndividualFigures\Fig1\D\Fig_1D_trace_data.csv");
        time = table.time_s;
...
%             figure; histogram2(diam_trace,flux_trace,diam_edges,flux_edges,'DisplayStyle','tile');
    ...
    if contains(subj_figs,' 2c')
        ...
        load(strcat(input_path, "\20230222 WT_PA_10\20230222PA10_5_rest_0um_allstats.mat"));
        dshallow = allstats.RadondEq_Outl;

        % load deep data
        load(strcat(input_path, "\20230222 WT_PA_10\20230222PA10_5_rest_380um_allstats.mat"));
...
%         counter = counter + 1;
    ...
    if contains(subj_figs,' 2d')
        ...
        clearvars -except log2C counter subj_session_id counter input_path subj_figs subtable subj nwb output_path S1 S2 S f phi phistd
    ...
    if contains(subj_figs,' 2e')
        clearvars -except log2C counter subj_session_id counter input_path subj_figs subtable subj nwb output_path
        load("\\dk-server.dk.ucsd.edu\jaduckwo\DataAnalysis\VesCorrPhase\Rui_2P\totampcomb_7_10_23.mat")
    ...
    %% Figure 3D
     ...
     if contains(subj_figs,' 4f')
         clearvars -except log2C counter subj_session_id counter subj_figs subtable subj nwb output_path
...
%% Start Figure 5C
     ...
     %% Figure 6D
     % in figure_save->fig
     %https://www.mathworks.com/help/images/ref/getimage.html
